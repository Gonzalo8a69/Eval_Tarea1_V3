...
def calcular_ipr(reservorio, pwf_actual):
    ...
    if pwf_actual >= pb:
        ...
        estado = "Flujo de Vogel"
        
    pwf_array = np.linspace(0, pr, 50)
    qo_array = [j * (pr - p) if p >= pb else qb + (j * pb / 1.8) * (1 - 0.2 * (p / pb) - 0.8 * (p / pb)**2) for p in pwf_array]
...
def calcular_hidrostatica(pozo):
    ...
    ph = gh * pozo.tvd
    dp = ph - pozo.pform
    condicion = "Sobrebalance" if dp > 0 else "Balance aproximado" if abs(dp) < 50 else "Bajo balance"
